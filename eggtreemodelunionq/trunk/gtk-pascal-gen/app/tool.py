...
def classunit(pclassname):
	...
	if pclassname == "DGdkEvent":
		return "uwrapgdknames"
	
	if pclassname == "DGdkColorspace":
			...
			if pclassname in psignalusedioc:
				for intorclass in psignalusedioc[pclassname]:
					if isInterface(intorclass):
						...
						if un not in arr:
							arr.append(un)
					elif intorclass.startswith("DGdkEvent"):
						if "ugdktypes" not in arr:
							arr.append("ugdktypes")
					elif intorclass == "DGdkRgbColor":
...
def c2ptype(typn, inreturn = False, canvar = False):
	...
	if typn.endswith("*") and typn[:-1] in c2pstructcopied:
		return "var " + c2pstructcopied[typn[:-1]]
...
def proptype1(prop):
	...
	#if pty == "DGtkBorder": return "Border" # FIXME
	#if pty == "DGtkRequisition": return "Requisition"
...
def getClassConstruct(pclass):
	...
	#if pclass.startswith("DGtkFileChooser.*Dialog"):
	# use DGtkFileChooserDialogCreateWrapped
	#if pclass.startswith("DGtkFileChooser.*Widget"):
	# use DGtkFileChooserWidgetCreateWrapped
	# blahblah dont think anything returns that though.
		
	pclassconstruct = "DGObjectDescendantGetOrCreateWrapper"
...
def getFNListReturnOverride(fn):
	...
	if returnoverride != None:
		...
		if returnoverride[0] == "array" and fn["return"] == returnoverride[1] + "*":
			...
			xarritemt = arritemt
			if xarritemt.startswith(classprefix): xarritemt = "T" + xarritemt

			preturn = arrayTypeOf(xarritemt)
			pclassconstruct = arritemt # fixme support complex types as like in "array" below
		...
		if returnoverride[0] == "array" and fn["return"] in ["GList*", "GSList*", "gchar**"]:
			...
			xarritemt = arritemt
			if xarritemt.startswith(interfaceprefix): xarritemt = "T" + xarritemt
...
def signalarg_cb(varname, signalparam): # signalparam: "arg": (prefix+type,name); "signal": signal, "argno": 1+
	...
	if varname == "signalhandlerargno":
		...
		if (argtype.startswith("DGObject") or argtype.startswith("IGObject")):
			if argname == "Sender":
				return "SELF"
			else:
				return "OBJECT"
...
def func_cb(varname, fn):
	...
	if varname == "preturnitemrawtype":
		...
		if arritemt.startswith(interfaceprefix):
			nativetype = "P" + arritemt[len(interfaceprefix):]
		elif arritemt == "DGtkAccelGroupEntry":
			nativetype = arritemt
	...
	if varname == "cfunctioncallparams":
		...
		for argtyp, argname in fn["args"]:
			...
			if fnover != None and len(fnover) > 0 and fnover[0].startswith("carrcount"):
				...
				o = pargtyp
				if pargtyp == "Pconst char": # gtk_label_set_text... sigh...
					pargtyp = "PChar"
					noat = True
					
				if pargtyp == "Pconst gchar":
					pargtyp = "PGChar"
					noat = True
					
				if pargtyp == "Tunsigned int": # gnome-canvas ...
					pargtyp = "guint"
					noat = True
					
				if pargtyp.startswith("Pconst "): # Pconst GdkColor
					pargtyp = pargtyp[7:]
				...
				if pcompargtyp == None or not isInterface(pcompargtyp):
					...
					if nocast == True:
						...
						if pargtyp.startswith("T"):
							xargtyp = enumprefix + pargtyp[1:]
		...
		try:
			if isMemberFN(fn):
				...
				a[0] = "P" + getInterfaceOfImplementationFN(fn) + "(Fobject)" # # hmm
		except:
			pass
