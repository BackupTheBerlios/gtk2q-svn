...
pnativeclasspointername = "P" + classname
...
def openDescFile(name):
	global gtkpascalgen_indir
	global path1
	try:
		return codecs.open(gtkpascalgen_indir + "/input/%s/%s.xml" % (path1, name), "r", "UTF-8")
	except exceptions.IOError, e:
		return codecs.open(gtkpascalgen_indir + "/overrides/%s/%s.xml" % (path1, name), "r", "UTF-8")

try:
	xxversion = openDescFile("version.txt").readlines()[0].strip()
except:
	pass
...
if classname == "GdkGC":
	...
	if not (getvaluesf in fns):
		ftype = "procedure"
		fnname = getvaluesf
		fnparams = ["gc: PGdkGc", "gcvalues: PGdkGcValues"]
		fnreturn = ""
		#fnlib =  # unchanged
		s = "%s %s(%s)%s; cdecl; external %s;" % (ftype, fnname, ";".join(fnparams),  fnreturn, fnlib)
		pexternfuncs.append(s)
